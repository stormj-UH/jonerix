...
| **`stormwall`** | `core` | 1.1.1 | MIT | any | `musl` | `rust` | Drop-in firewall front-end — accepts nft, OpenBSD pf (pfctl) and iptables/ip6tables CLI dialects (incl. iptables-save/restore) against the same in-kernel netfilter backend.  1.1.1 refreshes `--help` for all three dialects |
