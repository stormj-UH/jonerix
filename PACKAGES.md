...
| **`stormwall`** | `core` | 1.1.4 | MIT | any | `musl` | `rust` | Drop-in firewall front-end — accepts nft, OpenBSD pf (pfctl) and iptables/ip6tables CLI dialects (incl. iptables-save/restore) against the same in-kernel netfilter backend.  1.1.4: behavioural-soak triage — bare `-j REJECT` now emits ICMP_UNREACH+PORT (kernel was rejecting empty data); `--dport 1000:2000` lowers to a true Range expression; legacy `-s ! VALUE` negation form fixed |
