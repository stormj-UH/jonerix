...
| **`stormwall`** | `core` | 1.1.5 | MIT | any | `musl` | `rust` | Drop-in firewall front-end — accepts nft, OpenBSD pf (pfctl) and iptables/ip6tables CLI dialects against the same in-kernel netfilter backend.  1.1.5 feature-completeness pass: adds `-E` rename, `-c` compat, `-X` no-arg enumerate-and-delete, `--random-fully`, NOTRACK / NFQUEUE / TCPMSS / CT targets, LOG `--log-tcp-*`, `-m iprange/length/pkttype/connmark/tcpmss` matches.  220/220 cargo tests pass |
