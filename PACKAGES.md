...
| **`stormwall`** | `core` | 1.1.2 | MIT | any | `musl` | `rust` | Drop-in firewall front-end — accepts nft, OpenBSD pf (pfctl) and iptables/ip6tables CLI dialects (incl. iptables-save/restore) against the same in-kernel netfilter backend.  1.1.2 auto-creates user chains on `-A` and plants `/usr/sbin/iptables` + `/sbin/iptables` so dockerd's bridge driver finds it (1.1.1 refreshed `--help` for all three dialects) |
