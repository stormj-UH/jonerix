...
| **`stormwall`** | `core` | 1.1.3 | MIT | any | `musl` | `rust` | Drop-in firewall front-end — accepts nft, OpenBSD pf (pfctl) and iptables/ip6tables CLI dialects (incl. iptables-save/restore) against the same in-kernel netfilter backend.  1.1.3 auto-creates `-j USERCHAIN` jump targets too (1.1.2 only handled the operating chain), unblocking dockerd's `-I FORWARD -j DOCKER` post-cleanup path |
