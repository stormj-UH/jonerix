...
if ! command -v awk >/dev/null 2>&1; then
    echo "FATAL: awk is required by ci-build-aarch64.sh"
    exit 1
fi
...
if [ -n "$PKG_INPUT" ]; then
    ...
    # Append every recipe NOT already in build-order so the tail still
    # gets covered.  build-order has ~50 names; the repo has ~95 recipes.
